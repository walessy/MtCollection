...
void manageAlerts()
{
   if (alertsOn)
   {
      int whichBar = 1.0; if (alertsOnCurrent) whichBar = 0;
      if (vala[whichBar] != vala[whichBar+1])
      {
         if (vala[whichBar] ==  1.0) doAlert(whichBar,"crossed lower band");
         if (vala[whichBar] == -1.0) doAlert(whichBar,"crossed upper band");
      }
   }
}
