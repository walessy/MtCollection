...
int init()
{
      ...
      SetIndexBuffer(2,VIDYA);  SetIndexStyle(1,DRAW_LINE);
...
int start()
{
   ...
   i=0;
    ...
    for(i=limit; i>=0; i--){
      if (i < Bars - histper)
      {
         k =  iStdDev(NULL, 0, VIDYAperiod, 0, MODE_SMA, PRICE_CLOSE, i) / iStdDev(NULL, 0, histper, 0, MODE_SMA, PRICE_CLOSE, i);
         sc = (2.0 / (VIDYAperiod + 1));
         VIDYA[i] = k * sc * mom[i] + (1 - k * sc) * VIDYA[i+1];
      } else {
         VIDYA[i] = mom[i];
      }
   }   
