                ...
                const int &spread[])
{
   ...
   for (; i>=0 && !_StopFlag; i--)
   {
       ...
       sig[i]  = iCustomMa(SigMaMode,val[i],SigPeriod,i,rates_total,1);  
         ...
         // Draw a horizontal line when TSV Signal crosses TSV line
       if (i > 0 && ((sig[i] > val[i] && sig[i-1] <= val[i-1]) || (sig[i] < val[i] && sig[i-1] >= val[i-1])))
       {
           ObjectCreate("CrossoverLine", OBJ_TREND, 0, Time[i], val[i], Time[i-1], val[i-1]);
           ObjectSetInteger(0, "CrossoverLine", OBJPROP_COLOR, clrRed); // Change the color as needed
           ObjectSetInteger(0, "CrossoverLine", OBJPROP_RAY_RIGHT, 1);
           ObjectSetInteger(0, "CrossoverLine", OBJPROP_RAY_LEFT, 0);
       }
