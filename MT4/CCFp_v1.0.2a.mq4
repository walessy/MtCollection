...
//---- main cycle
   ...
   if(USD)
     {
       if (arrUSD[2]<arrUSD[1])  sl("SymbUSD","+", cur, Color_USD);
       if (arrUSD[2]>arrUSD[1])  sl("SymbUSD","-", cur, Color_USD);       
       if (arrUSD[2]==arrUSD[1]) sl("SymbUSD","=", cur, Color_USD);              
       cur += st;
     }
   if(EUR)
     {
       if (arrEUR[2]<arrEUR[1])  sl("SymbEUR","+", cur, Color_EUR);
       if (arrEUR[2]>arrEUR[1])  sl("SymbEUR","-", cur, Color_EUR);       
       if (arrEUR[2]==arrEUR[1]) sl("SymbEUR","=", cur, Color_EUR);        
       cur += st;
     }
   if(GBP)
     {
       if (arrGBP[2]<arrGBP[1])  sl("SymbGBP","+", cur, Color_GBP);
       if (arrGBP[2]>arrGBP[1])  sl("SymbGBP","-", cur, Color_GBP);       
       if (arrGBP[2]==arrGBP[1]) sl("SymbGBP","=", cur, Color_GBP);     
       cur += st;
     }
   if(CHF)
     {
       if (arrCHF[2]<arrCHF[1])  sl("SymbCHF","+", cur, Color_CHF);
       if (arrCHF[2]>arrCHF[1])  sl("SymbCHF","-", cur, Color_CHF);       
       if (arrCHF[2]==arrCHF[1]) sl("SymbCHF","=", cur, Color_CHF); 
       cur += st;
     }
   if(AUD)
     {
       if (arrAUD[2]<arrAUD[1])  sl("SymbAUD","+", cur, Color_AUD);
       if (arrAUD[2]>arrAUD[1])  sl("SymbAUD","-", cur, Color_AUD);       
       if (arrAUD[2]==arrAUD[1]) sl("SymbAUD","=", cur, Color_AUD); 
       cur += st;
     }
   if(CAD)
     {
       if (arrCAD[2]<arrCAD[1])  sl("SymbCAD","+", cur, Color_CAD);
       if (arrCAD[2]>arrCAD[1])  sl("SymbCAD","-", cur, Color_CAD);       
       if (arrCAD[2]==arrCAD[1]) sl("SymbCAD","=", cur, Color_CAD); 
       cur += st;
     }
   if(JPY)
     {
       if (arrJPY[2]<arrJPY[1])  sl("SymbJPY","+", cur, Color_JPY);
       if (arrJPY[2]>arrJPY[1])  sl("SymbJPY","-", cur, Color_JPY);       
       if (arrJPY[2]==arrJPY[1]) sl("SymbJPY","=", cur, Color_JPY);     
       cur += st;
     }
   if(NZD)
     {
       if (arrNZD[2]<arrNZD[1])  sl("SymbNZD","+", cur, Color_NZD);
       if (arrNZD[2]>arrNZD[1])  sl("SymbNZD","-", cur, Color_NZD);       
       if (arrNZD[2]==arrNZD[1]) sl("SymbNZD","=", cur, Color_NZD);         
       cur += st;
     }     
...
//|                                                                  |
//+------------------------------------------------------------------+
void sl(string IndicatoreName, string sym, int y, color col)
  {
   int window = WindowFind(Indicator_Name);
   string ID = Indicator_Name+Objs;
   int tmp = 10 + y;
   Objs++;
   if (ObjectFind(IndicatoreName)==-1)
   {
    if(ObjectCreate(IndicatoreName, OBJ_LABEL, window, 0, 0))
     {
       //ObjectDelete(ID);
       ObjectSet(IndicatoreName, OBJPROP_XDISTANCE, y + 35);
       ObjectSet(IndicatoreName, OBJPROP_YDISTANCE, 10);
       ObjectSetText(IndicatoreName, sym, 15, "Arial Black", col);
     }   
   }
   else
   {
