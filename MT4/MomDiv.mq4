...
//---- indicators
   ...
   SetIndexStyle(4,DRAW_LINE, STYLE_SOLID, 0); // EMA style
//----   
...
//----   
   ...
   SetIndexLabel(4, "EMA 8");
   //SetIndexEmptyValue(4,0.0);  

   IndicatorDigits(Digits + 2);
   IndicatorShortName("MomDiv"); //(" + MomPeriod + ")");
   return(0);
  }
...
void CalculateMomDiv(int i)
  {
   ...
   //ema[i] = iMA(NULL,0,8,0,MODE_EMA,PRICE_CLOSE,i);
   Comment(ema[i]);
...
void DrawIndicatorTrendLine(datetime x1, datetime x2, double y1, 
                            double y2, color lineColor, double style)
  {
   int indicatorWindow = WindowFind("MomDiv");//(" + MomPeriod + ")");
