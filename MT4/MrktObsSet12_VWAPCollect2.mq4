...
////#define PTDIndi "Indicators\\MarketObservtionSet12\\MrktObsSet12_VWAP_SD.ex4"
//#resource "\\"+PTDIndi
...
void OnTimer(){
   ...
   string sComment, sObjName1,sSQL;
   ...
   */   
   ...
   for(int dCell=0;dCell<SymbolsTotal(true);dCell++){
         ...
         vwap = iCustom(sComment,PERIOD_CURRENT,"ere\\Akif_Market_Statistics_Drag_v6_NoExpire",2,0);
         ...
         */
         ...
         //Print(sSQL);
         res = db.ExecDDL(sSQL);
         if (res != SQLITE_OK) PrintFormat("CouldN'T update vwap table database: (%d) %s", db.ErrCode(), db.ErrMsg());   
   }
}
